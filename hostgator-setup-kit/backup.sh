...
step "Snapshot das instâncias Evolution → $BACKUP_DIR/evolution-$ts.tgz"
vol="$(docker compose -f "$COMPOSE" config --volumes 2>/dev/null | grep -m1 evolution-instances || echo '')"
