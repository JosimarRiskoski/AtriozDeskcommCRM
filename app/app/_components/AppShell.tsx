...
export function AppShell({ sidebarCollapsed, children }: AppShellProps) {
  ...
  const isFullHeightWorkspace =
    pathname === "/app/inbox" || pathname.startsWith("/app/inbox/") || pathname.includes("/ai/followups/");

  return (
    <div className="flex h-screen min-h-0 w-full overflow-hidden bg-background">
      <Sidebar collapsed={sidebarCollapsed} />
      <div className={cn("flex h-screen min-h-0 flex-1 flex-col transition-[margin] duration-200", sidebarCollapsed ? "ml-16" : "ml-60")}>
        <TopBar />
        <main
          className={cn(
            "min-h-0 flex-1",
