...
-- ---- falhas de IA e fallback seguro (migration 0121) ----
...
-- ---- google calendar appointments (migration 0126) ----
...
begin
  if new.direction = 'outbound'
    ...
    perform public.fn_emit_notification(
      new.organization_id, 'send_failed', 'critical',
...
begin
  if new.archived_at is not null then
    ...
    if not exists (
      ...
      perform public.fn_emit_notification(
        new.organization_id, 'whatsapp_disconnected', 'critical',
        'WhatsApp desconectado',
        coalesce(new.status_reason, 'A conexao precisa ser verificada.'),
        '/app/connections', 'channel_session', new.id,
        'channel-down-' || new.id || '-' || extract(epoch from coalesce(new.last_status_change_at, now()))::bigint
      );
    end if;
  elsif not v_new_is_down and v_old_is_down then
...
-- 0138_conversation_command
-- Fonte única para as visões Fila/Automático, reutilizada pela API e pelo worker.
