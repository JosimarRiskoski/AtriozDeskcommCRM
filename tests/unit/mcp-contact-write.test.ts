...
import { crmAddContactNote, crmUpdateContact } from "@/lib/mcp/tools/contacts";
...
describe("crm_add_contact_note", () => {
  it("registra a observação no negócio aberto do mesmo contato", async () => {
    ...
    const fakeSupabase = {
      from(table: string) {
        const chain = {
          ...
          order: () => chain,
          limit: () => chain,
          maybeSingle: async () => table === "contacts"
    ...
    const args = z.object(crmAddContactNote.inputSchema).parse({ contact_id: CONTACT_ID, note: "Prefere atendimento depois das 18h." }) as never;
