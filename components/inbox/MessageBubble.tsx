...
function AckIndicator({
  ...
  if (status === "queued" || status === "sending") {
    label = "Enviando";
    indicator = <Clock size={12} weight="bold" className="text-current/70" aria-label={label} />;
  } else if ((ack ?? 0) >= 4 && messageType === "audio") {
    label = "Reproduzida";
    indicator = <Checks size={12} weight="bold" className="text-emerald-400" aria-label={label} />;
  } else if (status === "read") {
    label = "Lida";
    indicator = <Checks size={12} weight="bold" className="text-emerald-400" aria-label={label} />;
